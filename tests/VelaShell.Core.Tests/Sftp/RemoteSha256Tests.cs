...
public sealed class RemoteSha256Tests
{
    ...
    public void BuildCommand_RunsUnderShAndQuotesEveryPath()
    {
        ...
        StringAssert.Contains(command, "'/srv/a b.txt'");
        StringAssert.Contains(command, "'/srv/it'\\''s.txt'", "内嵌单引号必须写成 '\\''");
        StringAssert.Contains(command, "'/srv/$HOME;rm'", "单引号里的 $ 与 ; 不会被展开或断句");
    }
