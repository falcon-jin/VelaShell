...
| 状态 | 优先级 | 项 | 现状 | 要做什么 |
| :---: | :---: | --- | --- | --- |
| ✅ | — | ~~**X / Y / ZMODEM 的协议开关与默认传输方式**~~ | **已完成**（2026-09-14，`modem-plan.md`）：三种协议各自可在「设置 → 文件传输」启停，每条连接可在「高级 → 文件传输」覆盖（留「跟随全局」即不覆盖）；默认传输方式 SFTP / ZMODEM / YMODEM / XMODEM，由命令面板新增的「发送文件到远端…」「从远端接收文件…」消费；关掉 ZMODEM 即不再嗅探输出流。顺带移除了文件选择框的二次确认 | ⏳ **连接对话框的这一块目前只对 SSH 显示** —— 插件终端协议（Telnet / 串口）最用得上它，却拿不到「带不带终端」的能力位（`PluginProtocolTab` 不带，要走异步 `ResolveAsync`）。在补上那个能力位之前它们跟随全局。📄 velashell-docs 的《交互与界面规格》还没跟上 |
